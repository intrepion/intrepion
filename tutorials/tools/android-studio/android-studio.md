[Tutorials](../../tutorials.md) > [Tools](../tools.md) > Android Studio

# Android Studio

## Links

- [Website](https://developer.android.com/studio)
- [Documentation](https://developer.android.com/studio/intro)
- [Releases](https://developer.android.com/studio/releases)

## Versions

```bash
studio --version
```

should be something like

```text
$ studio --version
Android Studio Panda 4 | 2025.3.4 Patch 1
Build #AI-253.32098.37.2534.15336583
```

### Current

- Panda 4 | 2025.3.4 Patch 1 (May 2026)

### Previous

- Narwhal | 2025.1.1
- Arctic Fox | 2020.3.1

## Navigation

- [Android Studio Install](./install/android-studio-install.md)
